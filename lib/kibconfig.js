...
_commander2.default.command('copy <profile> <type> <id>').description('Copies the given exported file into a new location replacing it\'s ID.\n\n  Examples:\n\n    $ kibconfig copy pp dashboard Existing-dashboard --replace \'^Existing:New\'\n    $ kibconfig copy --deep pp dashboard Existing-dashboard --replace \'^Existing:New\' --ignore \'Navigation,Footer\' --dry-run').option('--deep', 'Deep copy', null).option('--replace <pattern>:<replacement>', 'Replacement', multiValueOption()).option('--ignore <id>,...', 'Ignored IDs', null).option('--dry-run', 'Dry run', null).action(function (profile, type, id, options) {
    var config = (0, _createConfig2.default)(_commander2.default, profile, options);
    var dataDirectory = new _DataDirectory2.default(config);
